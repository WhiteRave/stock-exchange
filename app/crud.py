...
# --- Instruments ---
async def create_instrument(db: AsyncSession, symbol: str, name: str, type):
    inst = Instrument(symbol=symbol, name=name, type=type)
    db.add(inst)
    await db.commit()
    await db.refresh(inst)
    return inst

async def list_instruments(db: AsyncSession, listed_only: bool = True):
    q = select(Instrument)
    if listed_only:
...
async def delist_instrument(db: AsyncSession, instrument_id: int):
    await db.execute(
        update(Instrument)
...
# --- Balances ---
async def get_balances(db: AsyncSession, user_id: int):
    res = await db.execute(select(Balance).where(Balance.user_id == user_id))
    return res.scalars().all()

async def adjust_balance(db: AsyncSession, user_id: int, instrument_id: int, delta: Decimal):
...
async def place_order(
    ...
    instrument_id: int, type,
    side, quantity: Decimal, price: Decimal = None
):
    order = Order(
        user_id=user_id,
...
async def list_active_orders(db: AsyncSession, user_id: int):
    res = await db.execute(
        select(Order).where(
            Order.user_id == user_id,
            Order.status.in_([OrderStatus.NEW, OrderStatus.PARTIAL])
...
async def get_order_book(db: AsyncSession, instrument_id: int, limit: int = 50):
